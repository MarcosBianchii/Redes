...
def handle_client(stream: RdpStream, storage: str):
    ...
    if msg.is_upload():
        ...
        try:
            ...
            error = Message.error(path, "There was a problem saving the file")
        ...
        try:
            ...
            error = Message.error(path, "The file does not exist")
            stream.send(error.encode())
...
if __name__ == "__main__":
    ...
    for stream in listener:
        thread = Thread(target=handle_client, args=(stream, storage))
        thread.start()
